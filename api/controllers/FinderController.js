...
const searchTripsWithFinder = async (req, res, next) => {
    try {
        const finder = await FinderModel.findById(req.params.explorerId);
        if (finder) {
            const trips = await finder.searchTrips();
...
export { createFinder, getFinder, updateFinder, deleteFinder, searchTripsWithFinder };